        ...
        try {
            ...
            if (msg != null) {
                ...
                if (contentEmail != null && contentEmail instanceof MimeMultipart) {
                    ...
                    String contenidoHtml = content + separadorAgenteHtml(MailHelper.toAddress(forward), forward.getSubject()) + originalHtml;
                    String contenidoText = content + separadorAgenteText(MailHelper.toAddress(forward), forward.getSubject()) + originalText;
                    ...
                    bodyText = new MimeBodyPart();
                    bodyHtml = new MimeBodyPart();
                    String contenidoHtml = content + separadorAgenteHtml(MailHelper.toAddress(forward), forward.getSubject()) + originalText;
                    String contenidoText = content + separadorAgenteText(MailHelper.toAddress(forward), forward.getSubject()) + originalText;
                    bodyText.setContent(contenidoText, "text/plain;charset=UTF-8");
                    bodyHtml.setContent(contenidoHtml, "text/html;charset=UTF-8");
                    alternative.addBodyPart(bodyText);
